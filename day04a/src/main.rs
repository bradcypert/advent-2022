...
trait IncludesVec<T> {
    fn includes_vec(&self, vec: &Vec<T>) -> bool;
}

impl IncludesVec<u32> for Vec<u32> {
    fn includes_vec(&self, vec: &Vec<u32>) -> bool {
        vec.iter().all(|x| self.contains(x))
    }
}
...
fn main() -> io::Result<()> {
    ...
    if let Some(file) = args.get(1) {
        ...
        for line in lines {
            ...
            if first_vec.includes_vec(&last_vec) || last_vec.includes_vec(&first_vec) {
                counter += 1;
            }
